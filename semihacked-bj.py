...
def blackj_options(p_chips, obj_de, obj_h, dealer_card):
    ...
    choice = str(input(f"[ [H]IT | [S]TAND | SURRENDER | [DOUBLE | HIY] : ")).lower()
    print("\n")
    if choice == "hit":
        print("you smell like an expired ***** who ****** like a ****** and ****** a ******")
        # hits(obj_de, obj_h)
...
def player_wins(obj_h, obj_d, obj_c):
    if any((obj_h.value == 21, obj_h.value > obj_d.value and obj_h.value < 21)):
        obj_c.win_bet()
        print("You fucking moron, I was supposed to win. Eat my balls and enjoy them!")
        return True
    return False
